var doshell_obj = {
    ...
    command_functions: {
        ...
        about: function(args) {
            if(args.length == 1)
                doshell_obj.push_lines(doshell_obj.ABOUT_MSG);
            else
                doshell_obj.error_functions.args_count(args[0]);
        },
        solut: function(args) {
            if(args.length == 1) {
                ...
                window.open("http://www.vilix.xyz/?p=asmhell",'_blank');
            }
            else
                doshell_obj.error_functions.args_count(args[0]);
        },
    ...
    'Welcome to ASM-Hell, version 5.9.12(8)-release (x64-torus) \n'+
    'available commands: \n'+
    '- HELP    (show this help) \n'+
    '- START n (start level n; first level is 0) \n'+
    '- PROGR   (show game progress and available levels)\n'+
/*    '- SOLUT   (author solutions, only last resort) \n'+ */
    '- ABOUT   (about this game) \n'+
    '- MAN     (ASM Hell manual; redundant,\n'+
    '           as everything is explained in-game)',
    ABOUT_MSG: 'A game made by @ViliX64, MFF student\n' +
               'Enjoy the game and say hi to me on Twitter!',
    SOLUT_MSG: 'Author solutions can be found here:\n' +
               'www.vilix.xyz/?p=amshell\n' +
               'In AMS Hell culture, this is considered cheating',
    MAN_MSG: 'http://www.vilix.xyz/?p=asmhell',
};
